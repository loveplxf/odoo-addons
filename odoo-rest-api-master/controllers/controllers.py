...
class OdooAPI(http.Controller):
    @http.route(
        '/auth_apikey/',
        type='json', auth='apikey', methods=["POST"], csrf=False)
    def authenticate_apikey(self):
        res = request.env['ir.http'].session_info()
        return res

    @http.route(
        '/auth_openid/',
        type='json', auth='openid', methods=["POST"], csrf=False)
    def authenticate_openid(self):
        res = request.env['ir.http'].session_info()
        return res
    ...
    def get_model_data(self, model, **params):
        ...
        if "filter" in params:
            ...
            for record in filters:
                record[:] = [value if value != 'False' else False for value in record]
                record[:] = [value if value != 'True' else True for value in record]
